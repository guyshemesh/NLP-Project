    ...
    for task_name in task_names:
        for base_model, model_dir in model_dirs.items():
            for model_name in model_names[base_model]:
                ...
                task = bigbench_tasks.get_task(dataset=task_name, number_json_examples=None, number_json_shots=number_of_shots)
                model = load_model(model_name, model_full_path, tokenizer_full_dir)
                scores_data = bigbench_tasks.evaluate_model_task(model, task)
                # each task has different evaluation matric provided as 'preferred_score'
                ...
                for score, sub_task in scores_and_subtasks:
                    new_row = pd.DataFrame([{'base_model': base_model, 'model_name': model_name, 'task_name': task_name + " " + sub_task, 'score': score}])
